...
function Upload() {
  ...
  const CLOUDINARY_CLOUD_NAME = "dnjvtjchd";
  const CLOUDINARY_UPLOAD_PRESET = "sihpreset";
    ...
    <div className="min-h-screen bg-[#EEEEEE] text-[#473472] relative overflow-hidden">
      <div className="relative z-10 max-w-4xl mx-auto px-6 pt-20">
        ...
        <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
          <Link
            to="/"
            className="inline-flex items-center gap-2 bg-[#53629E] text-white rounded-xl px-5 py-3 hover:bg-[#3F4B8B]"
          >
            <ArrowUpCircle className="rotate-180" size={22} />
            Back to Dashboard
          </Link>
        ...
        {/* Header */}
        <div className="text-center mt-12">
          <motion.h1 className="text-4xl md:text-5xl font-extrabold text-[#473472]">
            Upload Proposal Documents
          </motion.h1>
        </div>
        ...
        >
          ...
          {files.length > 0 ? (
            <div className="flex flex-col items-center">
              ...
              <p className="mt-3 font-semibold">{files[0].name}</p>
            ...
            <>
              ...
              <p className="font-semibold text-lg text-[#473472]">Choose Proposal Files</p>
              <p className="text-[#53629E] text-sm mt-1">
                Drag & drop or click to browse (PDF, DOCX, TXT, Images)
              </p>
            </>
          )}
        ...
        <div className="mt-10 bg-white p-6 rounded-xl shadow-md border border-[#53629E]">
          ...
          <label className="block mt-4 font-semibold text-[#473472]">Institution Name</label>
          <input
            type="text"
            className="w-full p-2 mt-2 border rounded-lg"
            value={institutionName}
        ...
        {files.length > 0 && (
          <motion.div className="mt-6 bg-white border border-[#53629E] p-6 rounded-xl shadow-md">
            <button
              onClick={handleUpload}
              disabled={loading}
              className={`mt-4 w-full px-4 py-3 font-bold rounded-lg transition-all ${
        ...
        {response.length > 0 && (
          <div className="mt-6 p-4 bg-green-100 text-green-800 rounded-lg">
            ...
            <ul className="mt-3 text-sm text-gray-800">
              {response.map((file, index) => (
                <li key={index}>
                  <a href={file.fileUrl} target="_blank" className="text-blue-700 underline">
                    {file.fileName}
                  </a>{" "}
                  ({(file.fileSize / 1024).toFixed(2)} KB)
                </li>
